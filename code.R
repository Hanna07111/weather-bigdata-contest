...
# 데이터프레임 변환 
df1 = data.frame(fire_firefighting[-c(1,2)]) # 처음 열인 num 제거 df2 = data.frame(fire_weather[-1]) 
...
colSums(df1=="") # fire_type_2 컬럼에만 "" 존재해 # 총 53718개 데이터 #-> 기타 + 임야 -> 해당 열 제거. fire_type_1 컬럼만 이용 df1 = subset(df1, select=-fire_type_2) 
...
df1$district_1 = as.factor(df1$district_1) ; levels(df1$district_1) df1$district_2 = as.factor(df1$district_2) 
df1$fire_type_1 = as.factor(df1$fire_type_1) ; levels(df1$fire_type_1) df1$ignition_factor_category_1 = as.factor(df1$ignition_factor_category_1) ; levels(df1$ignition_factor_category_1) 
df1$ignition_factor_category_2 = as.factor(df1$ignition_factor_category_2) df1$location_category_1 = as.factor(df1$location_category_1) ; levels(df1$location_category_1) 
df1$location_category_2 = as.factor(df1$location_category_2) ; levels(df1$location_category_2) 
df1$location_category_3 = as.factor(df1$location_category_3) str(df1) 
...
## 불필요 변수 제거 
# stn 변수 제거 (stn = aws 지점 코드 -> 지역과 같은 의미) df2 = subset(df2, select=-stn) 
...
# 1) 전체 지역 결측치 처리 시도 
# 데이터프레임을 district_1로 그룹화하고, 수치형 변수에 대해 선형 보간을 적용 # na.locf() 함수: NA를 선형 보간하기 전에 가장 가까운 비 NA 값으로 대체 df2_cleaned <- df2 %>%
group_by(district_1) %>% 
...
jeonbuk_weather <- df2 %>% filter(district_1 == "전북특별자치도") other_weather <- df2 %>% filter(district_1 != "전북특별자치도") 
...
busan_avg$ws_mean, busan_avg$ws_min, busan_avg$hm_max, 
busan_avg$hm_mean, busan_avg$hm_min) 
)) %>% 
ungroup() %>% 
arrange(euclidean_distance) 
...
# 5) 모든 지역의 df 합치기 
df2 <- bind_rows(df2_other, df2_jb_cleaned, df2_sj_cleaned, df2_bs_cleaned) summary(df2) # 모든 결측치 처리 완료 
View(df2) 
...
# 날짜형 변수를 POSIXct 형식으로 변환 
df3$tm_fc = as.POSIXct(df3$tm_fc, format = "%Y-%m-%d %H:%M:%S") df3$tm_ef = as.POSIXct(df3$tm_ef, format = "%Y-%m-%d %H:%M:%S") 
...
# cmd=3일 때 tm_fc가, cmd=1일 때 tm_fc와 tm_ef 사이에 있는지 확인 joined_data <- df3_1 %>% 
...
# 상관계수 행렬 시각화 
corrplot(cor_matrix, method = "color", type = "upper", tl.cex = 0.7, number.cex = 0.7) # 변수들 간의 상관관계가 높아 다중공선성 문제 발생 
...
# 상관계수 행렬 계산 
cor_matrix <- df_region %>% 
select(median_property_damage, rn_day,ws_mean,hm_mean) %>% cor(use = "complete.obs") 
...
df1_freq <- df1_freq %>% 
group_by(district_1) %>% 
mutate(freq_standardized = (count - mean(count)) / sd(count)) %>% ungroup() 

# 그래프 그리기 
ggplot() + 
# 막대 그래프 (df1_freq 데이터 사용) 
geom_bar(data = df1_freq, aes(x = factor(month), y = freq_standardized, fill = district_1), stat = "identity", position = "dodge") + 
# 꺾은선 그래프 (df2_removed_standardized 데이터 사용)
geom_point(data = df2_removed_standardized, aes(x = factor(month), y = hm_mean_standardized, color = "hm_mean (평균 상대습도)"), size = 1) + geom_line(data = df2_removed_standardized, aes(x = factor(month), y = hm_mean_standardized, group = district_1, color = "hm_mean (평균 상대습도)"), size = 1, linetype = "dashed") + 
...
labs(x = "Month", y = "Frequency Standardized (df1)", fill = "District", color = "Legend") + theme_minimal() + 
